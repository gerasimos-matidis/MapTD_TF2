...
if __name__ == '__main__':
    ...
    image_path = '/media/gerasimos/Νέος τόμος/Gerasimos/Toponym_Recognition/MapTD_General/MapTD_TF2/predictions/D0089-5235001.tiff' 
    
    if args.model:
        model = tf.keras.models.load_model(args.model) # NOTE: Need to modify it
    else:
        from maptd_model import maptd_model
        model = maptd_model()
        latest = tf.train.latest_checkpoint(args.checkpoint_dir)
        ...
        ckpt = tf.train.Checkpoint(generator=model) # NOTE: Raises Warning messages, because there are more objects stored, except the generator
        ckpt.restore(latest)
